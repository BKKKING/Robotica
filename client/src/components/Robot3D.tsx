...
export interface State { base: number; hombro: number; codo: number }
...
function RobotScene({ target }: { target: State }) {
  ...
  const shoulderPitch = -(interp.hombro - 90) * Math.PI/180
  const elbowPitch = -(interp.codo - 90) * Math.PI/180
...
export function Robot3D({ externalState }: Robot3DProps) {
  ...
  useEffect(() => {
    const loadInitialState = async () => {
      try {
        ...
        console.warn('无法加载机器人初始状态:', error)
      }
    }
